...
export function EvaluacionForm({
  ...
  return (
    ...
    <form onSubmit={submit} className="space-y-4 px-5 pt-5 pb-32 lg:pb-6">
      {/*
      ...
      */}
      <fieldset disabled={cab.cerrada} className="border-0 p-0 disabled:opacity-60">
        {/*
          Dos columnas desde lg: a la izquierda qué se evalúa y cuándo, a la
          derecha el detalle y las observaciones. En el celular es una sola, en
      ...
      */}
            ...
            >
              {areas.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  Todavía no agregaste áreas. Cada área trae sus evaluaciones y marcás las que
                  cumple.
                </p>
              ) : (
      ...
      {/* Footer fijo: el botón guardar siempre a la vista, no al final de un
          ...
          queda dentro de la columna del contenido, y el -mx-5 lo lleva de
          borde a borde de esa columna, por encima del px-5 del form. */}
      <div className="glass fixed inset-x-0 bottom-0 z-30 border-t border-border/60 pb-safe lg:sticky lg:-mx-5">
        <div className="mx-auto max-w-[480px] px-5 py-3 lg:mr-0">
          {/*
